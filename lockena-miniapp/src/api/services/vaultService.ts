import { decryptVaultItem } from "../../crypto/vaultItem";
...
import type { PasswordDto } from "../dto/vault-item/password.dto";
...
export const vaultService = {
  ...
  ): Promise<RequestState<GetVaultItemsDto<PasswordDto>>> {
    try {
      ...
      if (!masterKey) throw new Error("MasterKey is undefined");
      const passwords: PasswordDto[] = [];
      response.data.items.forEach(async (item) => {
        passwords.push({
          id: item.id,
          ...(await decryptVaultItem(masterKey, item)),
